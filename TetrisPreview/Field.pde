...
public class Field {
  ...
  public void logic() {
    if (!game_over) {
      if (timed) {
        if (lastMillis == -1) lastMillis = millis();
        else {
          if (millis()-lastMillis >= fallMillis){
            current.moveDown();
            checkDownCollision();
            lastMillis = millis();
          }
  ...
  private boolean checkMoveCollision() {
   ...
   for (int x=0;x<current.getWidth();x++) {
      for (int y=0;y<current.getHeight();y++) {
        if (current.getX() >= 0 && current.getY() >= 0 && t[x][y] && grid[x+current.getX()][y+current.getY()] != color(255)) return true;
      }
    }
    return false;
  }
  ...
  private boolean checkDownCollision() {
    ...
    if (current.getY()+current.getHeight() > field_height) {
      ...
      for (int x=0;x<current.getWidth();x++) {
        for (int y=0;y<current.getHeight();y++) {
          if (t[x][y] && grid[x+current.getX()][y+current.getY()] != color(255)) {
            current.moveUp();
            if (current.getY() <= 0) {
              game_over();
            } else {
  ...
  public boolean[][] getField() {
    boolean[][] out = new boolean[field_width][field_height];
    for (int x=0;x<field_width;x++) {
      for (int y=0;y<field_height;y++) {
        out[x][y] = grid[x][y]!=color(255); 
      }
    }
    ...
    for (int x=0;x<current.getWidth();x++) {
      for (int y=0;y<current.getHeight();y++) {
        if (x+current.getX() >= 0 && x+current.getX() < field_width && y+current.getY() >= 0 && y+current.getY() < field_height)
          out[x+current.getX()][y+current.getY()] = t[x][y];
      }
    }
  ...
  public double[] toInputArray() {
    ...
    for (int x = 0; x < field_width; x++) {
        for (int y = 0; y < field_height; y++) {
            out[x+y*field_width] = (grid[x][y] != color(255))?1d:0d;
        }
    }
